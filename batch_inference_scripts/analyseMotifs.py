...
def get_tokens(data_file, filt_size=5):
    summary, X = parseFile(data_file)
    if len(summary) == 0:
        return []
 
    # get a mode filter on the preds
    Xmode = copy.deepcopy(X)
    for k in range(filt_size, X.shape[0]-filt_size):
        Xmode[k] = scipy.stats.mode(X[k-filt_size:k+filt_size]).mode[0]

    Xdiff = Xmode[1:] - Xmode[:-1]    
    Xidx = np.where(Xdiff != 0)[0]
    tokens = Xmode[Xidx]
    return tokens
...
if __name__ == '__main__':
    ...
    # TYPE I
    #groupHI = {'FC-A': ['17202339', '17202342', '17202346', '17202341'], 'FC-B':['17202345', '17202338'], 'FC-C':['17202346', '17202345', '17202339', '17202342'], 'FC-D':['17202342', '6394837']}
    #groupLOW = {'FC-A': ['17202338', '17202345'], 'FC-B':['17202346', '17202346', '17202341', '17202339','17202342'], 'FC-C':['17202341', '17202338'], 'FC-D':['17202341', '17202339', '6394836', '6394841']}

    # TYPE II
    groupHI = {'FC-A': ['17202338'] , 'FC-B':['17202345', '17202338', '17202346', '17202341'], 'FC-C':['17202341', '17202346'], 'FC-D':['17202342', '6394837','17202341', '17202339', '6394836', '6394841']}
    groupLOW = {'FC-A': ['17202339', '17202342', '17202346', '17202341', '17202345'], 'FC-B':['17202339','17202342'], 'FC-C':[ '17202338', '17202345', '17202339', '17202342'], 'FC-D':[]}


    group = 'postext'
    # need a list of folder names for a given experiment and phase
    #datasets = [('FC-A', 'preexposure', 'Trap'), ('FC-B', 'preexposure'), ('FC-C', 'preexposure'), ('FC-D', 'preexposure')]
    #datasets = [('FC-A', 'postcond'), ('FC-B', 'postcond'), ('FC-C', 'postcond'), ('FC-D', 'postcond')]
    #datasets = [('FC-A', 'postret'), ('FC-B', 'postret'), ('FC-C', 'postret'), ('FC-D', 'postret')]
    datasets = [('FC-A', 'postext'), ('FC-B', 'postext'), ('FC-C', 'postext'), ('FC-D', 'postext')]
    ...
    for dset in datasets:
        ...
        for exp_name in exp:
            cur_exps.extend([x for x in pred_datafiles if exp_name in x.parts and (int(x.name.split('_')[4][:-1]) <= 6 or int(x.name.split('_')[4][:-1]) >= 18)])

    vids_groupHI = []
    for key in groupHI.keys():
        for cam_idx in groupHI[key]:
            vids = [x for x in cur_exps if (cam_idx in str(x)) and (key in str(x))]
            vids_groupHI.extend(vids)

    vids_groupLOW = []
    for key in groupLOW.keys():
        for cam_idx in groupLOW[key]:
            vids = [x for x in cur_exps if (cam_idx in str(x)) and (key in str(x))]
            vids_groupLOW.extend(vids)

    print('total: {}'.format(len(cur_exps))) 
    r = Parallel(n_jobs=32, verbose=10)(delayed(get_tokens)(x, filt_size=5) for x in vids_groupHI)
    X = zip(*r)
    pickle.dump(r, open('HI_hcage_FC_{}_tokens.p'.format(group), 'wb'))

    r = Parallel(n_jobs=32, verbose=10)(delayed(get_tokens)(x, filt_size=5) for x in vids_groupLOW)
    X = zip(*r)
    pickle.dump(r, open('LOW_hcage_FC_{}_tokens.p'.format(group), 'wb'))
